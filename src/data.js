(function(root){
 ...
 function teamSize(value){return [3,5,7].includes(Number(value))?Number(value):3;}
 function randomRoster(count,random=Math.random){const result=[];while(result.length<count){const pool=tanks.slice();for(let i=pool.length-1;i>0;i--){const j=Math.min(i,Math.floor(random()*(i+1)));[pool[i],pool[j]]=[pool[j],pool[i]];}result.push(...pool.slice(0,count-result.length));}return result;}
 function roleRoster(count,random=Math.random){const roles=['capturer','captureSupport','tactician','push','tacticalSupport','flanker'],deck=roles.slice();for(let i=deck.length-1;i>0;i--){const j=Math.floor(random()*(i+1));[deck[i],deck[j]]=[deck[j],deck[i]];}const result=Array.from({length:count},(_,i)=>deck[i%deck.length]);if(count>=4&&!result.includes('capturer'))result[0]='capturer';return result;}
 ...
 function defaults(){return {teamSize:3,version:3,purchaseOrder:['0-1'],modules:Object.fromEntries(tanks.map(t=>[t.id,moduleLevels()])),owned:{'0-1':true},updated:0,selected:'0-1',nation:0,silver:0,xp:0,levels:Object.fromEntries(tanks.map(t=>[t.id,1])),battles:0,wins:0,tutorial:false,settings:{volume:.45,quality:'high',difficulty:'normal',sensitivity:1,renderScale:1,shadows:true,postprocessing:true,cameraShake:true},map:'training'};}
 function clean(raw){const d=defaults();if(!raw||typeof raw!=='object')return d;const number=(x,min,max,fallback)=>Number.isFinite(x)?Math.max(min,Math.min(max,x)):fallback;for(const k of ['silver','xp','battles','wins','updated'])d[k]=Math.floor(number(raw[k],0,1e14,0));for(const t of tanks){d.levels[t.id]=Math.floor(number(raw.levels?.[t.id],1,5,1));if(raw.owned?.[t.id]===true||(!raw.owned&&d.levels[t.id]>1))d.owned[t.id]=true;}for(const t of tanks){d.modules[t.id]=moduleLevels(raw.modules?.[t.id]??d.levels[t.id]);d.levels[t.id]=Math.max(...Object.values(d.modules[t.id]));}const requested=tanks.find(t=>t.id===raw.selected),fallback=tanks.find(t=>d.owned[t.id])||tanks[1];d.selected=requested&&d.owned[requested.id]?requested.id:fallback.id;d.nation=tanks.find(t=>t.id===d.selected).n;d.purchaseOrder=[...new Set([...(Array.isArray(raw.purchaseOrder)?raw.purchaseOrder:[]),...Object.keys(d.owned)])].filter(id=>d.owned[id]);d.teamSize=teamSize(raw.teamSize);d.tutorial=raw.tutorial===true;d.map=maps[raw.map]?raw.map:'training';const s=raw.settings||{};d.settings.volume=number(s.volume,0,1,.45);d.settings.sensitivity=number(s.sensitivity,.25,2.5,1);d.settings.quality=['low','high'].includes(s.quality)?s.quality:'high';d.settings.difficulty=['easy','normal','hard'].includes(s.difficulty)?s.difficulty:'normal';d.settings.renderScale=number(s.renderScale,.5,1.5,1);for(const k of ['shadows','postprocessing','cameraShake'])d.settings[k]=s[k]!==false;return d;}
