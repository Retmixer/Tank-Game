/* Steel Frontier. Three.js r160; original tank art and local licensed assets. */
(() => {
'use strict';
...
function building(x,z,w,d,h,industrial=false){
 const asset=window.MapAssets?.create('building-'+['a','b','c','d','e','f'][Math.abs(Math.round(x*3+z))%6],w,h,d);
...
function rock(x,z,r){const asset=window.MapAssets?.create('rock_large'+['A','B','C'][Math.abs(Math.round(x+z))%3],r*2.4,r*1.6,r*2);if(asset){asset.position.set(x,height(x,z)-.3,z);world.add(asset);obstacle(asset,x,z,r*1.2);return;}const o=mesh(new THREE.IcosahedronGeometry(r,2),mat(arena===D.maps.desert?0x9d886d:arena===D.maps.winter?0x8a9b9e:0x777e72),world,x,height(x,z)+r*.46,z);o.scale.set(1.2,.8,1);o.rotation.set(rand()*.4,rand()*6,.2);obstacle(o,x,z,r*1.08);}
...
function buildMap(){
 ...
 if(window.MapAssets&&arena!==D.maps.desert)for(let i=0;i<42;i++){const x=(rand()-.5)*size*.78,z=(rand()-.5)*size*.78;if(Math.abs(x)<22||Math.abs(z)<15||obstacles.some(o=>Math.hypot(o.x-x,o.z-z)<o.r+9))continue;const tree=window.MapAssets.create(arena===D.maps.winter?'tree_pineDefaultA':'tree_oak',6,9+rand()*4,6);tree.position.set(x,height(x,z)-.06,z);world.add(tree);}
...
function moveTank(t,dt){
 ...
 const tankAt=(x,z)=>tanks.find(o=>o!==t&&(o.alive||o.wreck)&&Math.abs(p.y-o.group.position.y)<3&&Remaster.overlaps(t,x,z,o));
 const impact=other=>{
  const normal=new V(p.x-other.group.position.x,0,p.z-other.group.position.z).normalize(),vx=other.velocityX||Math.sin(other.yaw)*other.speed||0,vz=other.velocityZ||Math.cos(other.yaw)*other.speed||0;
  const closing=-((t.velocityX-vx)*normal.x+(t.velocityZ-vz)*normal.z),impulse=D.contactImpulse(t.spec.mass,other.alive?other.spec.mass:Infinity,closing);
  t.velocityX+=normal.x*impulse/t.spec.mass;t.velocityZ+=normal.z*impulse/t.spec.mass;
  if(other.alive){other.velocityX=vx-normal.x*impulse/other.spec.mass;other.velocityZ=vz-normal.z*impulse/other.spec.mass;other.speed=other.velocityX*Math.sin(other.yaw)+other.velocityZ*Math.cos(other.yaw);other.slip=other.velocityX*Math.cos(other.yaw)-other.velocityZ*Math.sin(other.yaw);}
  if(closing>1&&(t.isPlayer||other.isPlayer)){shake=Math.max(shake,Math.min(.4,closing*.045));sound('hit',Math.min(.5,closing*.08),.7);}
 };
 let other=tankAt(p.x+dx,p.z);if(blocked(p.x+dx,p.z,r))t.velocityX=0;else if(other)impact(other);else p.x+=dx;
 other=tankAt(p.x,p.z+dz);if(blocked(p.x,p.z+dz,r))t.velocityZ=0;else if(other)impact(other);else p.z+=dz;
