...
function updateMapCard(){ $('mapPreview').innerHTML='<div>СЛУЧАЙНАЯ КАРТА<small>Полигон · пустыня · зимний завод</small></div>';updateModeCard();}
function updateModeCard(){const size=D.teamSize($('modeSelect').value);$('modeName').textContent='◈ КОМАНДНЫЙ БОЙ';$('modeSize').textContent=size+' × '+size;$('modeDescription').textContent='Уничтожьте противника или захватите базу. Карта выбирается случайно.';$('modeNote').textContent='ВЫ И '+(size-1)+' СОЮЗНЫХ БОТОВ ПРОТИВ '+size+' БОТОВ';$('battleBtn').innerHTML='В БОЙ <span>→</span>';}
...
function startBattle(options={}){
 ...
 if(options instanceof Event)options={};initAudio();$('modal').hidden=true;$('garage').hidden=true;$('hud').hidden=false;state='battle';paused=false;teamSize=D.teamSize(save.teamSize);battleDifficulty=save.settings.difficulty;const maps=Object.keys(D.maps);save.map=options.map&&D.maps[options.map]?options.map:maps[Math.floor(Math.random()*maps.length)];arena=D.maps[save.map];setupScene(save.map);buildMap();tanks=[];markers.clear();$('markers').innerHTML='';elapsed=0;battleTime=arena.time;capture=0;viewYaw=0;viewPitch=0;mouseDown=false;aiming=false;shake=0;record={damage:0,kills:0,spotted:0,capture:0,life:0,win:false,survived:false};
...
function bind(){
 ...
 const launch=()=>startBattle();if(window.BattleFlow)window.BattleFlow.launch=launch;$('battleBtn').onclick=()=>{if(window.BattleFlow)window.BattleFlow.open();else launch();};$('upgradeBtn').onclick=()=>{if(D.upgradeModule(save,save.selected,selectedModule)){persist();renderGarage();sound('ui');toast('Модуль «'+D.moduleNames[selectedModule]+'»: уровень '+roman[save.modules[save.selected][selectedModule]-1]);}};$('settingsBtn').onclick=settings;$('helpBtn').onclick=help;$('detailsBtn').onclick=details;$('pauseBtn').onclick=pause;$('shelfPrev').onclick=()=>$('tankCards').scrollBy({left:-240,behavior:'smooth'});$('shelfNext').onclick=()=>$('tankCards').scrollBy({left:240,behavior:'smooth'});$('botDifficulty').onchange=()=>{save.settings.difficulty=$('botDifficulty').value;persist();};$('modeSelect').onchange=()=>{save.teamSize=D.teamSize($('modeSelect').value);persist();updateModeCard();};
