...
function startBattle(options={}){
 ...
 if(matchMode==='lan'){
  ...
  const roster=D.randomRoster(5);let botIndex=0;for(let i=0;i<3;i++){const s=i===1?D.stats(chosen(),save.modules[save.selected]):D.stats(roster[botIndex++],save.levels[save.selected]);const t=spawn(s,0,i,i===1);if(i===1)player=t;}
  for(let i=0;i<3;i++)spawn(D.stats(roster[botIndex++],Math.max(1,save.levels[save.selected]-(save.settings.difficulty==='easy'?1:0))),1,i);
 }
 viewYaw=player.yaw;
...
function handleNetMessage(message){if(!net)return;if(message.type==='joined'){net.players=message.data.players;lobbyView();}else if(message.type==='ready'&&net.role==='host'){net.guest=message.data.spec;net.guestName=message.data.name;lobbyView();}else if(message.type==='start'&&net.role==='guest'){startBattle({...message.data,lan:true});}else if(message.type==='input'&&net.role==='host'){net.input=message.data||{};}else if(message.type==='snapshot'&&net.role==='guest'){applyNetSnapshot(message.data);}else if(message.type==='end'&&state==='battle'){endBattle(message.data.winner===player.team,message.data.reason||'Сетевой бой завершён.',message.data.results);}else if(message.type==='leave'){toast('Соперник покинул лобби');}}
...
function updateRemote(t,dt){const i=net?.input||{},accel=(i.w?1:0)-(i.s?1:0),turn=(i.a?1:0)-(i.d?1:0);steerTank(t,turn,dt);accelerateTank(t,accel,dt,turn);moveTank(t,dt);t.group.rotation.y=t.yaw;t.turretYaw=approach(t.turretYaw,Number.isFinite(i.yaw)?i.yaw:t.turretYaw,t.spec.turret*dt);t.turret.rotation.y=angle(t.turretYaw-t.yaw);t.remoteAim=!!i.aiming;t.aimBloom=D.aimStep(t.aimBloom,t.remoteAim,Math.min(1,Math.abs(t.speed)/Math.max(1,t.spec.speed)+Math.abs(turn)*.7),t.spec.aim,dt);if(Array.isArray(i.target)&&i.target.length===3){const target=new V(i.target[0],i.target[1],i.target[2]);aimGun(t,target,dt);if(i.fire)fire(t,target);}}
function netSnapshot(){return {time:battleTime,capture,countdown,tanks:tanks.map(t=>({team:t.team,damage:t.damage,kills:t.kills,spotted:t.spotted,shotSerial:t.shotSerial||0,hitSerial:t.hitSerial||0,x:t.group.position.x,y:t.group.position.y,z:t.group.position.z,yaw:t.yaw,turret:t.turretYaw,hp:t.hp,alive:t.alive,reload:t.reload,speed:t.speed,trackTime:t.trackTime,brokenSide:t.brokenSide,vy:t.suspension?.vy||0,grounded:t.suspension?.grounded!==false,pitch:t.group.rotation.x,roll:t.group.rotation.z}))};}
function applyNetSnapshot(data){if(!data?.tanks||state!=='battle')return;battleTime=data.time;capture=data.capture;for(const s of data.tanks){const t=tanks.find(v=>v.team===s.team);if(!t)continue;const previous=t.group.position.clone(),alpha=t.isPlayer?.22:.62;t.group.position.lerp(new V(s.x,s.y,s.z),alpha);t.yaw=angle(t.yaw+angle(s.yaw-t.yaw)*alpha);t.turretYaw=angle(t.turretYaw+angle(s.turret-t.turretYaw)*alpha);t.group.rotation.y=t.yaw;t.turret.rotation.y=angle(t.turretYaw-t.yaw);if(t.alive&&!s.alive)makeWreck(t);t.trackTime=s.trackTime||0;t.brokenSide=s.brokenSide;t.trackYaw=s.yaw;if((s.shotSerial||0)>(t.shotSerial||0))shotFeedback(t);t.shotSerial=s.shotSerial||0;if((s.hitSerial||0)>(t.hitSerial||0)&&t.isPlayer){shake=Math.max(shake,.35);sound('hit');}t.hitSerial=s.hitSerial||0;t.damage=s.damage||0;t.kills=s.kills||0;t.spotted=s.spotted||0;if(t.isPlayer){record.damage=t.damage;record.kills=t.kills;}t.hp=s.hp;t.alive=s.alive;t.reload=s.reload;if(t.suspension){t.suspension.y=t.group.position.y;t.suspension.vy=s.vy||0;t.suspension.grounded=s.grounded!==false;}
...
function endBattle(win,reason,authoritativeResults){
 if(state!=='battle')return;
 const winner=win?player.team:1-player.team,results=authoritativeResults||battleResults(winner),own=results.find(r=>r.id===player.id),reward=own?.reward||D.reward(record);
 if(matchMode==='lan'&&net?.role==='host')netSend('end',{winner,reason,results});
