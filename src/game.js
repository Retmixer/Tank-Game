...
let teamSize=3,battleDifficulty='normal',countdown=0,countdownClock=null,armorView=false,aimHit=null,garageSection='hangar',researchNation=0,selectedModule='gun';
...
function setupScene(kind){
 ...
 const hemi=new THREE.HemisphereLight(kind==='garage'?0xe8f5ef:kind==='winter'?0x9db7e4:0xe2eeff,kind==='garage'?0x59645f:0x655c43,kind==='garage'?1.65:kind==='winter'?.95:1.25);scene.add(hemi);
...
function updateMapCard(){ $('mapPreview').textContent=D.maps[save.map].name;updateModeCard();}
function updateModeCard(){const size=D.teamSize($('modeSelect').value);$('modeName').textContent='◈ КОМАНДНЫЙ БОЙ';$('modeSize').textContent=size+' × '+size;$('modeDescription').textContent='Уничтожьте противника или захватите базу. Карта и сложность ботов выбираются перед боем.';$('modeNote').textContent='ВЫ И '+(size-1)+' СОЮЗНЫХ БОТОВ ПРОТИВ '+size+' БОТОВ';$('battleBtn').innerHTML='В БОЙ <span>→</span>';}
...
function building(x,z,w,d,h,industrial=false){
 const asset=industrial&&w<16?window.MapAssets?.create('building-'+['a','b','c','d','e','f'][Math.abs(Math.round(x*3+z))%6],w,h,d):null;
 if(asset){const support=footprintSupport(x,z,w,d),g=new THREE.Group(),foundation=Math.max(.8,support.high-support.low+.55);g.position.set(x,support.high-.08,z);world.add(g);box(g,w+.35,foundation,d+.35,0,-foundation/2+.12,0,0x62665d);g.add(asset);obstacle(g,x,z,Math.hypot(w,d)*.5+.2);return;}
const support=footprintSupport(x,z,w,d),foundation=Math.max(.8,support.high-support.low+.55),g=new THREE.Group();g.position.set(x,support.high-.08,z);world.add(g);box(g,w+.75,foundation,d+.75,0,-foundation/2+.12,0,arena===D.maps.desert?0x756b59:0x59625f);box(g,w,h,d,0,h/2,0,industrial?0x6a7779:arena===D.maps.desert?0xc2ae86:0xa0a391);box(g,w+.3,.25,d+.3,0,h,0,0x515c5d);
...
function rock(x,z,r){
 const geometry=new THREE.IcosahedronGeometry(r,3),vertices=geometry.attributes.position;
 for(let i=0;i<vertices.count;i++){const a=vertices.getX(i),b=vertices.getY(i),c=vertices.getZ(i),shape=1+.14*Math.sin(a*1.7+c*.8)*Math.cos(b*2.1)+.09*Math.sin(c*2.8+x);vertices.setXYZ(i,a*shape,b*(.88+.12*shape),c*shape);}
 geometry.computeVertexNormals();const support=footprintSupport(x,z,r*1.7,r*1.6);
 const material=Remaster.material(arena===D.maps.desert?0xb69a71:arena===D.maps.winter?0x919ca1:0x858a80,'rock');
 const o=mesh(geometry,material,world,x,support.low+r*.28,z);o.scale.set(1.2,.8,1);o.rotation.y=rand()*Math.PI*2;obstacle(o,x,z,r*1.08);
}
...
function naturalBoundary(){
 const edge=arena.size*.468,stone=Remaster.material(arena===D.maps.desert?0x92785b:arena===D.maps.winter?0x7f9299:0x68746b,'rock'),step=26,shared=new THREE.IcosahedronGeometry(1,2);
 const add=(x,z,i,vertical)=>{const r=8+(i%4)*1.8,o=mesh(shared,stone,world,x,height(x,z)+r*.25,z);o.scale.set(r*(vertical?.9:1.45),r*.8,r*(vertical?1.45:.9));o.rotation.set(.12*(i%3),i*.77,.08*(i%2));obstacle(o,x,z,r*.76);};
...
// Authored districts use the same deterministic layout for every battle format.
function buildDistricts(){
 ...
 if(desert){
  ...
  for(const side of [-1,1]){
   for(const x of [-105,105]){building(x,side*size*.29,16,22,7,false);supplies(x+22,side*size*.27);tower(x*1.6,side*size*.26);}
   for(const x of [-55,55]){building(x,side*size*.39,14,18,6,false);supplies(x+16,side*size*.37);}
...
function buildMap(){
 ...
 window.MapDressing?.populate({world,arena,height,obstacles,obstacle,building});
 ...
 Remaster.environment(world,arena,height,rand,save.settings.quality);Remaster.vegetation(world,arena,height,obstacles,save.settings.quality);world.updateMatrixWorld(true);makeNavigation();
}
...
function spawnPoint(team,index,width){
 const side=team===0?-1:1,offset=(index-(teamSize-1)/2)*14,cx=side*arena.size*.39+offset,cz=side*arena.size*.39-offset;
 for(let ring=0;ring<18;ring++)for(let k=0;k<(ring?16:1);k++){const angle=k*Math.PI/8,x=cx+Math.cos(angle)*ring*7,z=cz+Math.sin(angle)*ring*7;if(side*x<arena.size*.2||side*z<arena.size*.2||blocked(x,z,Math.max(4.5,width)))continue;const support=footprintSupport(x,z,7,7);if(support.high-support.low>1.4)continue;if(tanks.some(t=>Math.hypot(t.group.position.x-x,t.group.position.z-z)<14))continue;return {x,z};}
 throw new Error('Не найдена свободная стартовая позиция');
}
function spawn(spec,team,index,isPlayer=false){const model=buildTank(spec,team);const {x,z}=spawnPoint(team,index,model.width);model.group.position.set(x,height(x,z),z);world.add(model.group);const t={...model,spec,team,isPlayer,id:team+'-'+index,damage:0,kills:0,spotted:0,captureTime:0,yawRate:0,throttle:0,recoil:0,hp:spec.hp,yaw:team===0?Math.PI/4:-Math.PI*3/4,turretYaw:team===0?0:Math.PI,reload:1+index*.25,speed:0,alive:true,target:null,think:rand()*.4,path:[],navTime:0,seen:false,aimTime:0,aimBloom:1,revealTime:0,stuck:0,trackTime:0,name:isPlayer?'Вы':(team===0?['Кедр','Сокол','Риф','Барс','Ветер','Туман','Рубеж'][index]:['Коршун','Кремень','Буран','Гром','Шторм','Ворон','Ирбис'][index])};model.group.rotation.y=t.yaw;Remaster.suspension(t,height,1/60);model.hitMeshes.forEach(o=>o.userData.tank=t);if(!isPlayer)t.brain=createBrain();tanks.push(t);if(!isPlayer){const el=document.createElement('div');el.className='marker '+(team===0?'friendly':'');$('markers').append(el);markers.set(t,el);}return t;}
function startBattle(options={}){
 if(!save.owned[save.selected]){toast('Сначала исследуйте и купите танк');return;}countdown=5;countdownClock=null;aiming=false;$('hud').classList?.remove?.('aiming');Object.keys(keys).forEach(k=>keys[k]=false);$('countdown').hidden=false;$('countNumber').textContent='5';
 if(options instanceof Event)options={};initAudio();$('modal').hidden=true;$('garage').hidden=true;$('hud').hidden=false;state='battle';paused=false;teamSize=D.teamSize(save.teamSize);battleDifficulty=save.settings.difficulty;save.map=options.map&&D.maps[options.map]?options.map:save.map;arena=D.maps[save.map];setupScene(save.map);buildMap();tanks=[];markers.clear();$('markers').innerHTML='';elapsed=0;battleTime=arena.time;capture=0;viewYaw=0;viewPitch=0;mouseDown=false;aiming=false;shake=0;record={damage:0,kills:0,spotted:0,capture:0,life:0,win:false,survived:false};
 const roster=D.randomRoster(teamSize*2-1);let botIndex=0;
 for(let team=0;team<2;team++)for(let i=0;i<teamSize;i++){const isPlayer=team===0&&i===1,level=Math.max(1,save.levels[save.selected]-(team===1&&battleDifficulty==='easy'?1:0));const spec=isPlayer?D.stats(chosen(),save.modules[save.selected]):D.stats(roster[botIndex++],level);const t=spawn(spec,team,i,isPlayer);if(isPlayer)player=t;}
 for(let team=0;team<2;team++){const bots=tanks.filter(t=>t.team===team&&!t.isPlayer),roles=D.roleRoster(bots.length);bots.forEach((t,i)=>t.brain.role=roles[i]);}
 viewYaw=player.yaw;
 $('playerName').textContent=player.spec.name.toUpperCase();$('mapName').textContent=arena.name.toUpperCase();$('killfeed').innerHTML='';$('hitMessage').textContent='';$('tutorialHint').textContent=save.tutorial?'':'W / S — движение, A / D — поворот корпуса. Shift — оптика, ЛКМ — выстрел. База A находится в центре карты.';camera.position.copy(player.group.position).add(new V(0,7,-13));Platform.gameplay(true);updateHUD();
}
...
function updateBot(t,dt){
 const p=t.group.position;t.think-=dt;t.navTime-=dt;const difficulty=battleDifficulty;const cfg=difficulty==='easy'?{reaction:.9,spread:.055,aggression:.7}:difficulty==='hard'?{reaction:.26,spread:.016,aggression:1}:{reaction:.55,spread:.031,aggression:.87};
 t.brain??=createBrain();
 ...
 if(Math.hypot(goal.x-p.x,goal.z-p.z)>5){if(t.navTime<=0){t.path=route(p.x,p.z,goal.x,goal.z);t.navTime=1.6+rand();}while(t.path.length&&Math.hypot(t.path[0].x-p.x,t.path[0].z-p.z)<3.7)t.path.shift();let next=t.path[0]||goal;const vx=next.x-p.x,vz=next.z-p.z,len=Math.hypot(vx,vz)||1,ux=vx/len,uz=vz/len;
  ...
  const desired=Math.atan2(next.x-p.x,next.z-p.z);const turn=angle(desired-t.yaw);steerTank(t,Math.max(-1,Math.min(1,turn*2.5)),dt);accelerateTank(t,t.stuck>1.5?-.6:cfg.aggression*t.brain.pace*Math.max(.08,1-Math.abs(turn)/1.8),dt,turn);if(!moveTank(t,dt)){t.stuck+=dt;steerTank(t,t.team===0?1:-1,dt);if(t.stuck>1.5){t.path=[];t.navTime=0;if(t.stuck>3)t.stuck=0;}}else t.stuck=0;}else{steerTank(t,0,dt);accelerateTank(t,0,dt);moveTank(t,dt);}
...
function steerTank(t,input,dt){
 if(t.trackTime>0||t.suspension?.grounded===false){t.yawRate=0;return;}
 t.yawRate=D.steeringStep(t.spec,t.yawRate||0,input,t.speed,dt,groundSurface());t.yaw+=t.yawRate*dt;
}
...
function moveTank(t,dt){
 ...
 if(contact){
  ...
  const distance=Math.hypot(ox,oz)||1,nx=-ox/distance,nz=-oz/distance;
  const closing=-((t.velocityX-(contact.velocityX||0))*nx+(t.velocityZ-(contact.velocityZ||0))*nz);
  const impulse=D.contactImpulse(t.spec.mass,contact.alive?contact.spec.mass:Infinity,closing);
  if(contact.alive&&impulse>0){contact.velocityX=(contact.velocityX||0)-nx*impulse/contact.spec.mass;contact.velocityZ=(contact.velocityZ||0)-nz*impulse/contact.spec.mass;contact.speed=contact.velocityX*Math.sin(contact.yaw)+contact.velocityZ*Math.cos(contact.yaw);contact.slip=contact.velocityX*Math.cos(contact.yaw)-contact.velocityZ*Math.sin(contact.yaw);}
  if(closing>1&&(t.isPlayer||contact.isPlayer)){shake=Math.max(shake,Math.min(.4,closing*.045));sound('hit',Math.min(.5,closing*.08),.7);}
...
function muzzlePos(t){t.group.updateMatrixWorld(true);return t.gunPivot.localToWorld(new V(0,0,t.barrelLength+.5-(t.recoil||0)));}
...
function damageTank(t,amount,owner,point,normal){if(!t.alive||t.team===owner.team)return;let damage=Math.round(amount*(1-t.spec.armor/100)*(.91+Math.random()*.18));const crit=Math.random()<.08; if(crit){damage=Math.round(damage*1.25);}damage=Math.min(t.hp,damage);t.hp-=damage;owner.damage=(owner.damage||0)+damage;t.hitSerial=(t.hitSerial||0)+1;if(t.suspension)t.suspension.vroll+=(Math.random()<.5?-1:1)*Math.min(.22,damage/t.spec.mass*.02);if(t.isPlayer&&damage>0){capture=0;hitText('ЗАХВАТ СБРОШЕН');}addImpactHole(t,point,normal,amount);if(owner.isPlayer){record.damage+=damage;hitText((crit?'КРИТИЧЕСКОЕ ПОПАДАНИЕ  ':'ПОПАДАНИЕ  ')+damage);sound('hit');}if(t.isPlayer){sound('hit',.85,.85);shake=Math.max(shake,.3+damage/t.spec.hp*.3);$('damageFlash').style.opacity=1;setTimeout(()=>$('damageFlash').style.opacity=0,160);}burst(point,0xffd487,18,.23);smoke(point,0x403d38,.32,1.2);if(t.hp<=0){owner.kills=(owner.kills||0)+1;makeWreck(t);burst(t.group.position.clone().add(new V(0,1,0)),0xdb8942,36,1.1);for(let i=0;i<8;i++)smoke(t.group.position.clone().add(new V((Math.random()-.5)*2,1+Math.random(),(Math.random()-.5)*2)),0x343532,.7,1.7);sound('boom',t.isPlayer?1:.55);if(owner.isPlayer){record.kills++;hitText('ПРОТИВНИК УНИЧТОЖЕН');}const line=document.createElement('div');line.textContent=`${owner.name} › ${t.name} · ${t.spec.name}`;line.style.color=owner.team===0?'#99d2c6':'#e99880';$('killfeed').prepend(line);while($('killfeed').children.length>5)$('killfeed').lastChild.remove();if(t.isPlayer){record.life=elapsed;$('tutorialHint').textContent='Танк уничтожен. Бой продолжается.';document.exitPointerLock?.();mouseDown=false;}}}
 ...
 for(let i=bullets.length-1;i>=0;i--){
  ...
  if(hit){
   ...
   if(t&&!t.wreck){
    ...
    else if(normal){sound('hit',t.isPlayer?.7:.25,1.3);if(t.isPlayer)shake=Math.max(shake,.16);const bounce=D.ricochetVelocity(b.velocity,normal,incidence,b.ricochets);if(bounce){
   ...
   }else{if(player.group.position.distanceTo(hit.point)<90)sound('hit',.3/(1+player.group.position.distanceTo(hit.point)/25),.75);burst(hit.point,t?0xffe1a0:0xffdf9a,14,.16);if(!t)smoke(hit.point,0x6d675b,.25,1.1);consumed=true;}
  }
...
function updateCamera(dt){const focus=player.alive?player:tanks.find(t=>t.alive&&t.team===player.team)||player;const p=focus.group.position;if(!player.alive)viewYaw+=dt*.045;const scoped=aiming&&player.alive,d=scoped?-Math.max(3,focus.length*.55):12,targetDistance=aiming?90:23;const target=p.clone().add(new V(Math.sin(viewYaw)*targetDistance,2.2+viewPitch*targetDistance,Math.cos(viewYaw)*targetDistance));const desired=p.clone().add(new V(-Math.sin(viewYaw)*d,scoped?2.65:6.4,-Math.cos(viewYaw)*d));const pivot=p.clone().add(new V(0,2.6,0));ray.set(pivot,desired.clone().sub(pivot).normalize());ray.far=desired.distanceTo(pivot);const obstruction=ray.intersectObjects(solidMeshes,false)[0];if(obstruction)desired.copy(ray.ray.at(Math.max(1,obstruction.distance-.6),new V()));desired.y=Math.max(desired.y,height(desired.x,desired.z)+1.5);if(scoped)camera.position.copy(desired);else camera.position.lerp(desired,1-Math.exp(-dt*10));if(save.settings.cameraShake===false){shake=0;player.cameraKick=0;}if(shake>0){camera.position.x+=(Math.random()-.5)*shake;camera.position.y+=(Math.random()-.5)*shake;shake*=Math.exp(-dt*11);}if(player.cameraKick){camera.position.addScaledVector(new V(-Math.sin(viewYaw),.16,-Math.cos(viewYaw)),player.cameraKick);player.cameraKick*=Math.exp(-dt*9);}const zoom=player?.spec.zoom||2.5,cameraFov=aiming?Math.max(12,60/zoom):60;camera.fov=THREE.MathUtils.lerp(camera.fov,cameraFov,1-Math.exp(-dt*(aiming?7:10)));camera.updateProjectionMatrix();camera.lookAt(target);}
...
function battleResults(winner){
 return D.standings(tanks.map(t=>({id:t.id,team:t.team,name:t.name,tank:t.spec.name,damage:t.damage,kills:t.kills,spotted:t.spotted,capture:t.captureTime,survived:t.alive,win:t.team===winner}))).map(r=>({...r,reward:D.reward(r)}));
}
function endBattle(win,reason,abandoned=false){
 if(state!=='battle')return;
 const winner=win?player.team:1-player.team,results=battleResults(winner),own=results.find(r=>r.id===player.id);if(abandoned&&own)own.reward=D.reward({abandoned:true});const reward=own?.reward||D.reward({...record,abandoned});
 updateHUD();state='result';paused=false;Platform.gameplay(false);document.exitPointerLock?.();mouseDown=false;aiming=false;record.win=win;record.survived=player.alive;
 save.silver+=reward.silver;save.xp+=reward.xp;save.battles++;if(win)save.wins++;persist();if(engineGain)engineGain.gain.value=0;window.BattleAudio?.update(player,save.map,false);$('scoreboard').hidden=true;
 const rows=results.map(r=>`<tr class="${r.id===player.id?'result-own':''}"><td>${r.place}</td><td><strong>${r.id===player.id?'Вы':escapeText(r.name==='Вы'?'Соперник':r.name)}</strong><small>${escapeText(r.tank)} · ${r.team===player.team?'Союзник':'Противник'}</small></td><td>${r.damage}</td><td>${r.kills}</td><td>+${r.reward.silver}</td><td>+${r.reward.xp}</td></tr>`).join('');
 showModal(`<div class="battle-results"><div class="result-title"><small>ОПЕРАЦИЯ ЗАВЕРШЕНА</small><h2>${win?'Победа':'Поражение'}</h2><p>${escapeText(reason)}</p></div><p class="result-place">Ваше место: <b>${own?.place||'—'} / ${results.length}</b></p><div class="results-scroll"><table class="results-table"><caption>Результаты всех участников</caption><thead><tr><th>Место</th><th>Командир / техника</th><th>Урон</th><th>Убито</th><th>Серебро</th><th>Опыт</th></tr></thead><tbody>${rows}</tbody></table></div><p class="help-note">Места по урону, затем по уничтожениям. Равные результаты — одинаковое место.</p><div class="reward"><div>+${reward.silver} ◉<small>ВАШЕ СЕРЕБРО</small></div><div>+${reward.xp} ✦<small>ВАШ ОПЫТ</small></div></div><p class="help-note">Бонус за место: +${reward.parts.placement||0} серебра и +${abandoned?0:Math.max(0,7-(own?.place||7))*30} опыта. Остальное — участие, урон, уничтожения, победа, разведка, захват и выживание.</p><button id="returnGarage" class="primary">В АНГАР →</button></div>`);
 const returnToGarage=()=>hangar();
 $('returnGarage').onclick=returnToGarage;
 if(window.BattleMenus)window.BattleMenus.results({win,reason,own,reward,rows,abandoned,map:arena.name,duration:elapsed,teamSize:results.length,onGarage:returnToGarage,onAgain:()=>{returnToGarage();window.BattleFlow?.open();}});
}

function showModal(html){$('modalContent').innerHTML=html;$('modal').hidden=false;}
function pause(){if(state!=='battle'||paused)return;paused=true;countdownClock=null;mouseDown=false;aiming=false;$('hud').classList?.remove?.('aiming');Object.keys(keys).forEach(k=>keys[k]=false);Platform.gameplay(false);document.exitPointerLock?.();if(engineGain)engineGain.gain.value=0;showModal(`<div class="pause-screen"><div class="pause-meta">ПОЛЕ БОЯ<small>${escapeText(arena.name)} · Командный бой</small></div><h2>ПАУЗА</h2><p>При досрочном выходе вы не получите опыт и серебро.</p><div class="pause-actions"><button id="resume" class="menu-button selected">ПРОДОЛЖИТЬ ИГРУ</button><button id="pauseSettings" class="menu-button">НАСТРОЙКИ</button><button id="pauseGarage" class="menu-button">ВЕРНУТЬСЯ В АНГАР</button><button id="leave" class="menu-button">ПОКИНУТЬ БОЙ</button></div><div class="pause-bottom"><button id="pauseBack" class="text-btn"><kbd>ESC</kbd> Назад</button><span>СРАЖАЙСЯ ДАЛЬШЕ. ПОБЕДА БЛИЖЕ.</span></div></div>`);$('resume').onclick=resume;$('pauseBack').onclick=resume;$('pauseGarage').onclick=()=>{endBattle(false,'Досрочный выход: опыт и серебро не начисляются.',true);hangar();};$('pauseSettings').onclick=settings;$('leave').onclick=()=>endBattle(false,'Досрочный выход: опыт и серебро не начисляются.',true);}
function resume(){paused=false;$('modal').hidden=true;initAudio();Platform.gameplay(true);}
function settings(){const inBattle=state==='battle';if(window.BattleMenus){window.BattleMenus.settings(save.settings,values=>{save.settings=values;$('botDifficulty').value=values.difficulty;persist();applyQuality();if(master)master.gain.value=values.volume;},()=>{if(inBattle){paused=false;pause();}else $('modal').hidden=true;});return;}showModal(`<div class="eyebrow">ПАРАМЕТРЫ</div><h2>Настройки</h2><label for="volume">Громкость</label><input id="volume" type="range" min="0" max="1" step=".05" value="${save.settings.volume}"><label for="quality">Графика</label><select id="quality"><option value="high">Максимальная — PBR, AO, тени 4K</option><option value="low">Производительность — без постобработки</option></select><label for="difficulty">Боты — со следующего боя</label><select id="difficulty"><option value="easy">Лёгкие</option><option value="normal">Обычные</option><option value="hard">Сложные</option></select><label for="sensitivity">Чувствительность мыши</label><input id="sensitivity" type="range" min=".25" max="2.5" step=".05" value="${save.settings.sensitivity}"><p class="help-note">Звук двигателя: Nayckron, qubodup · CC BY 3.0. <a href="assets/credits.html" target="_blank" rel="noopener">Авторы моделей и звуков</a></p><button id="saveSettings" class="primary">СОХРАНИТЬ</button><button id="closeSettings" class="text-btn">← Назад</button>`);$('quality').value=save.settings.quality;$('difficulty').value=save.settings.difficulty;$('volume').oninput=()=>{initAudio();if(master)master.gain.value=+$('volume').value;};const back=()=>{if(master)master.gain.value=save.settings.volume;if(inBattle){paused=false;pause();}else $('modal').hidden=true;};$('closeSettings').onclick=back;$('saveSettings').onclick=()=>{save.settings={volume:+$('volume').value,quality:$('quality').value,difficulty:$('difficulty').value,sensitivity:+$('sensitivity').value};persist();applyQuality();back();};}
function applyQuality(){renderer.shadowMap.enabled=save.settings.quality==='high'&&save.settings.shadows!==false;renderer.setPixelRatio(Math.min(devicePixelRatio,save.settings.quality==='high'?2:1)*(save.settings.renderScale||1));if(scene)scene.traverse(o=>{if(o.material)o.material.needsUpdate=true;});}
function details(){const s=D.stats(chosen(),save.modules[save.selected]);showModal(`<div class="eyebrow">ПАСПОРТ МАШИНЫ · ${s.nation.toUpperCase()}</div><h2>${s.name}</h2><table>${[['Класс',s.cls],['Уровень',s.level+'/5'],['Прочность',s.hp+' HP'],['Урон',s.damage],['Перезарядка',s.reload+' с'],['Скорость',Math.round(s.speed*3.6)+' км/ч'],['Поворот корпуса',(s.turn*180/Math.PI).toFixed(0)+' °/с'],['Поворот башни',(s.turret*180/Math.PI).toFixed(0)+' °/с'],['Снижение урона бронёй',s.armor+'%'],['Дальность обзора',s.view+' м'],['Обзор в оптике',Math.round(D.sightRange(s,true))+' м'],['Оптическое увеличение',s.zoom.toFixed(2)+'×'],['Точность',Math.round((1-s.spread)*1000)/10+'%'],['Разброс',s.spread.toFixed(3)+' рад'],['Время сведения',s.aim.toFixed(2)+' с'],['Прочность ходовой',s.tracks+' (паспорт)'],['Маскировка',Math.round(s.camo*100)+'%']].map(([k,v])=>`<tr><td>${k}</td><td>${v}</td></tr>`).join('')}</table><p class="help-note">Оптика включается удержанием Shift. Увеличение зависит от класса, нации и уровня модернизации.</p><button id="closeDetails" class="secondary">← Назад в ангар</button>`);$('closeDetails').onclick=()=>$('modal').hidden=true;}
function help(){showModal('<div class="eyebrow">КУРС МОЛОДОГО КОМАНДИРА</div><h2>Займите свой рубеж</h2><p>Выберите бой 3 × 3, 5 × 5 или 7 × 7. Союзники и противники — боты с разными ролями. Карта выбирается случайно.</p><div class="tutorial-keys"><span><kbd>W / S</kbd> вперёд / назад</span><span><kbd>A / D</kbd> поворот корпуса</span><span><kbd>Мышь</kbd> башня и камера</span><span><kbd>ЛКМ</kbd> выстрел</span><span><kbd>Shift</kbd> оптика танка</span><span><kbd>Пробел</kbd> выстрел</span><span><kbd>Tab</kbd> состав команд</span><span><kbd>Esc</kbd> пауза</span></div><p>Удерживайте Shift для оптического прицела. Его увеличение зависит от характеристик выбранной машины и растёт с модернизацией.</p><p>За досрочный выход из боя опыт и серебро не начисляются.</p><button id="closeHelp" class="primary">ПОНЯТНО →</button>');$('closeHelp').onclick=()=>{$('modal').hidden=true;};}
...
function bind(){
 ...
 const launch=()=>startBattle();if(window.BattleFlow)window.BattleFlow.launch=launch;$('battleBtn').onclick=()=>{if(window.BattleFlow)window.BattleFlow.open();else launch();};$('upgradeBtn').onclick=()=>{if(D.upgradeModule(save,save.selected,selectedModule)){persist();renderGarage();sound('ui');toast('Модуль «'+D.moduleNames[selectedModule]+'»: уровень '+roman[save.modules[save.selected][selectedModule]-1]);}};$('settingsBtn').onclick=settings;$('helpBtn').onclick=help;$('detailsBtn').onclick=details;$('pauseBtn').onclick=pause;$('shelfPrev').onclick=()=>$('tankCards').scrollBy({left:-240,behavior:'smooth'});$('shelfNext').onclick=()=>$('tankCards').scrollBy({left:240,behavior:'smooth'});$('mapSelect').onchange=()=>{if(D.maps[$('mapSelect').value]){save.map=$('mapSelect').value;persist();updateMapCard();}};$('botDifficulty').onchange=()=>{save.settings.difficulty=$('botDifficulty').value;persist();};$('modeSelect').onchange=()=>{save.teamSize=D.teamSize($('modeSelect').value);persist();updateModeCard();};
...
window.gameStatus=()=>({state,paused,countdown,armorView,owned:{...save.owned},map:save.map,tank:save.selected,mode:teamSize+'x'+teamSize,teamSize,difficulty:battleDifficulty,silver:save.silver,levels:{...save.levels},modules:structuredClone(save.modules),elapsed,camera:{fov:camera?.fov,zoom:camera?.zoom,aiming},player:player?{team:player.team,hp:player.hp,x:player.group.position.x,z:player.group.position.z,reload:player.reload,zoom:player.spec.zoom}:null,audio:window.BattleAudio?.status(),teams:tanks.map(t=>({id:t.id,tank:t.spec.id,maxHp:t.spec.hp,damage:t.damage,kills:t.kills,team:t.team,hp:t.hp,alive:t.alive,role:t.brain?.role,behavior:t.brain?.mode,x:t.group.position.x,z:t.group.position.z})),drawCalls:renderer?.info.render.calls});
