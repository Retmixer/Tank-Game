...
(() => {
 const T=THREE;
 const texture=new T.TextureLoader().load('assets/models/industrial-colormap.png');texture.colorSpace=T.SRGBColorSpace;
 function create(name,w,h,d){
  ...
  if(window.Remaster)for(const mesh of group.children){
   const geometry=mesh.geometry,p=geometry.attributes.position,n=geometry.attributes.normal,uv=geometry.attributes.uv,c=geometry.attributes.color;
   const tree=name.startsWith('tree'),rock=name.startsWith('rock'),green=c.getY(0)>c.getX(0)*1.05;
   const kind=rock?'rock':tree?(green?'leaf':'wood'):'brick';
   // Project by each face's dominant normal in metres; atlas UVs would smear a PBR surface.
   for(let i=0;i<p.count;i++){const x=p.getX(i)*group.scale.x,y=p.getY(i)*group.scale.y,z=p.getZ(i)*group.scale.z,ax=Math.abs(n.getX(i)),ay=Math.abs(n.getY(i)),az=Math.abs(n.getZ(i));uv.setXY(i,(ax>ay&&ax>az?z:x)/3,(ay>ax&&ay>az?z:y)/3);}
   const tint=new T.Color().setRGB(c.getX(0),c.getY(0),c.getZ(0));
   mesh.material=window.Remaster.material(tint.getHex(),kind);geometry.deleteAttribute('color');
  }
  return group;
 }
