(() => {
 ...
 const captions={easy:['ЛЁГКИЕ','МЕДЛЕННЕЕ РЕАКЦИЯ · НИЖЕ ТОЧНОСТЬ'],normal:['ОБЫЧНЫЕ','СБАЛАНСИРОВАННЫЙ БОЙ'],hard:['СЛОЖНЫЕ','БЫСТРАЯ РЕАКЦИЯ · ТОЧНЫЙ ОГОНЬ']};
 document.querySelectorAll('.map-choice').forEach(button=>{const [title,subtitle]=captions[button.dataset.difficulty];button.innerHTML=`<span class="map-photo" aria-hidden="true"></span><span class="map-caption"><strong>${title}</strong><small>${subtitle}</small></span><span class="choice-action">ВЫБРАТЬ</span>`;});
 let busy=false,selected='normal',pending=0;
 ...
 function select(map){selected=map;document.querySelectorAll('.map-choice').forEach(b=>{const active=b.dataset.difficulty===map;b.setAttribute('aria-pressed',String(active));b.querySelector('.choice-action').textContent=active?'ВЫБРАНО':'ВЫБРАТЬ';});$('botDifficulty').value=map;$('botDifficulty').onchange?.();}
 function hideSetup(){$('battleSetup').hidden=true;$('garage').inert=false;}
 function close(){if(busy)return;hideSetup();$('battleBtn').focus();}
 function open(){if(busy)return;select($('botDifficulty').value||selected);$('setupMode').value=$('modeSelect').value||'3';$('garage').inert=true;$('battleSetup').hidden=false;$('setupBack').focus();}
 ...
 document.querySelectorAll('.map-choice').forEach(button=>button.onclick=()=>select(button.dataset.difficulty));
